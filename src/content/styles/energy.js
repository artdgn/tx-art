...
export function energyDraw(sketch) {
  ...
  sketch.setup = function() {
    ...
    sketch.createCanvas(Math.min(maxSize, sketch.windowWidth), Math.min(maxSize, sketch.windowHeight), sketch.WEBGL);
    sketch.shader(mainShader);
  ...
  sketch.draw = function() {
    ...
    mainShader.setUniform("time", phase + sketch.millis() / 1000.0);
  ...
  function randomFromHash() {
    ...
    phase = sketch.map(decPairs[0], 0, 255, 0, 100);
    //phase =  map(random(0,255), 0, 255, 0, 100);
    let ranDelay = sketch.round(sketch.map(decPairs[0], 0, 255, 1, 3));
    ...
    let ranSpeed = sketch.round(sketch.map(decPairs[1], 0, 255, 1, 3));
    ...
    //print(speed);
    let ranColor = sketch.map(decPairs[2], 0, 255, 0, 1);
    //let ranColor = map(random(0,255), 0, 255, 0, 1);

    hueC = ranColor;

    let ranDim = sketch.round(sketch.map(decPairs[3], 0, 255, 0, 1));
    //let ranDim = round(map(random(0,255), 0, 255, 0, 1));

    if (ranDim == 0) {
      // let dimOne = round(map(random(0,255), 0, 255, 0, 9));
      let dimOne = sketch.round(sketch.map(decPairs[0], 0, 255, 0, 9));
      //let dimTwo = round(map(random(0,255), 0, 255, 0, 9));
      let dimTwo = sketch.round(sketch.map(decPairs[1], 0, 255, 0, 9));
      dimLayerOne = [dimOne, dimOne, dimOne];
      dimLayerTwo = [dimTwo, dimTwo, dimTwo];
    }
    if (ranDim == 1) {

      dimLayerOne = [sketch.round(sketch.map(decPairs[0], 0, 255, 0, 9)), sketch.round(sketch.map(decPairs[1], 0, 255, 0, 9)), sketch.round(sketch.map(decPairs[2], 0, 255, 0, 9))];
      dimLayerTwo = [sketch.round(sketch.map(decPairs[3], 0, 255, 0, 9)), sketch.round(sketch.map(decPairs[4], 0, 255, 0, 9)), sketch.round(sketch.map(decPairs[5], 0, 255, 0, 9))];

    }

    let ranBounce = sketch.round(sketch.map(decPairs[4], 0, 255, 1, 6));
    //let ranBounce = round(map(random(0,255), 0, 255, 1, 6));

    bounceSpeed = ranBounce; //range1-6
    morphSpeeds = [sketch.round(sketch.map(decPairs[0], 0, 255, 1, 6)), sketch.round(sketch.map(decPairs[1], 0, 255, 1, 6)), sketch.round(sketch.map(decPairs[2], 0, 255, 1, 6))];
    let ranPlot = sketch.round(sketch.map(decPairs[5], 0, 255, 1, 6))
  ...
  function getShader(_renderer) {
   ...
   `;
    ...
    const frag = `
       ...
       const float WIDTH = ${Math.min(maxSize, sketch.windowWidth)}.0;
       const float HEIGHT = ${Math.min(maxSize, sketch.windowHeight)}.0;
